<template>
  <form @submit.prevent="sendData" class="post-deleter">
    <h3 class="post-deleter__title">Видалення поста</h3>
    <label class="post-deleter__label" for="services">Пост: </label>
    <select v-model="id"
            id="posts"
            class="post-deleter__input-field post-deleter__input-field--select"
            name="post"
            required>
    ...
    <button class="post-deleter__submit" type="submit">Видалити</button>

    <success-or-fail-modal-window
        class="post-deleter__result-window result-window"
  ...
  methods: {
    sendData() {
      ...
      axios.post(this.link, formData)
          .then(response => {
            this.result.text = "Пост успішно видалений";
          ...
          .catch(error => {
            if (error.response && error.response.data && error.response.data.errors) {
              ...
              this.result.text = "Не вдалося видалити пост";
